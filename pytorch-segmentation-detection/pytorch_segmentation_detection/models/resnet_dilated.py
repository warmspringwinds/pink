...
class Resnet18_4s(nn.Module):
    
    
    def __init__(self, num_classes=1000):
        
        super(Resnet18_4s, self).__init__()
...
class Resnet34_8s_multigrid(nn.Module):
    
    
    def __init__(self, num_classes=1000):
        
        super(Resnet34_8s_multigrid, self).__init__()
    ...
    def forward(self, x, test=False):
        ...
        if test:
            ...
            x = nn.functional.upsample_bilinear(input=x, size=(400, 640))
        else:
...
class Resnet18_8s_multigrid_openeds(nn.Module):
    
    
    def __init__(self, num_classes=1000):
        
        super(Resnet18_8s_multigrid_openeds, self).__init__()
        
        # Load the pretrained weights, remove avg pool
        # layer and get the output stride of 8
        resnet34_8s = models.resnet18(fully_conv=True,
                                       pretrained=True,
                                       output_stride=8,
                                       remove_avg_pool_layer=True,
                                       additional_blocks=0,
